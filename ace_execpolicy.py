...
设计动机（见 docs/ADR-002-executor-boundary.md）：
...
class Verdict:
    ...
    `reason` 与 `reason_key` 是**两个受众**，不是同一句话的两种写法：

    - `reason` 给**模型**（`tools/file_tools.py` 把它拼进 403 的 `message`，
      而 `message` 会进模型上下文）。它固定中文，且刻意不随界面语言变：系统提示词
      是中文写的，把这条链路的语言跟着 UI 一起切，等于让模型的输入语言由用户的
      界面偏好决定。`execution_layer.DENIAL_INSTRUCTIONS` 上方那段注释定的就是
      这条边界（"给模型看的文案不进 i18n"），这里沿用。
    - `reason_key` / `reason_args` 给**人**：展示层（`agent_runner` 的确认框）
      拿键去查 locales，于是 en/ja 界面不会再出现
      `Reason: 命令需要确认 (rule: shell_syntax)` 这种半英半中。

    为什么不是"UI 侧建一张中文 reason → 键的映射表"：那等于把判据挂回中文子串，
    正是 `DenialKind` 那一轮刚拆掉的东西 —— 文案一改，映射静默失效，不报错。

    只有 prompt 档填 `reason_key`：forbidden 档不问人、allow 档不拦，两者的
    `reason` 目前没有任何人类展示出口，先填键只会得到一批查不到也测不到的死键。
    """
...
_FORBIDDEN_RULES: Sequence[Tuple[str, re.Pattern, str]] = (
    ...
    ("shutdown", re.compile(r"\b(shutdown|reboot|halt|poweroff)\b"), "关机/重启"),
)
...
def evaluate_command(cmd: str,
    ...
    ok, offender = _paths_within(argv, root)
    if not ok:
