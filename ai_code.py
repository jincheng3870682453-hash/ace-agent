...
from agent_runner import (ModelProvider, TOOLS, content_to_tool_protocol,  # noqa: E402
                          ...
                          make_cli_approval_hook, StallTracker,
                          render_result, render_tool_result, sanitize_plain_content,
                          tool_calls_to_protocol,
                          DETAIL_TAP, execution_detail_lines)
...
#
# name 是**品牌**，故意不进语言包：用户要拿它去和服务商控制台/官网对照，
# 我们自己译一个"Zhipu AI"出来只会对不上（智谱官网自己在 BigModel / Z.ai 之间混用）。
# 括号里的**功能限定词**才是我们写的说明文字，走 note_key → t() 出去；
# 不拆开的话，en/ja 的提供商列表里会夹一句中文。渲染见 _provider_label。
PROVIDERS = [
    {"id": "zhipu", "name": "智谱 GLM", "note_key": "provider_note_anthropic_endpoint",
    ...
    {"id": "zhipu-openai", "name": "智谱 GLM", "note_key": "provider_note_openai_endpoint",
    ...
    {"id": "openrouter", "name": "OpenRouter", "note_key": "provider_note_aggregator",
    ...
    {"id": "ollama", "name": "Ollama", "note_key": "provider_note_local",
...
def _provider_label(p: Dict) -> str:
    ...
    单独抽出来是因为 /provider 列表和 /config 向导两条路径都要渲染同一份数据 ——
    各自拼一次的话，改了一处漏了另一处，界面上就会出现两种叫法。
    """
    note = p.get("note_key")
    return p["name"] + (t(note) if note else "")
...
def _pip_install_with_fallbacks(target: str) -> bool:
    ...
    for src in sources:
        name = src[-1] if src else t("pip_default_source")
        print(t("pip_trying", name=name))
        try:
...
def _model_error_hint(e: Exception) -> str:
    """根据 HTTP 错误码返回排查指引（401 等常见错误不再只甩英文原文）"""
...
def _config_sanity_hints(cfg: Dict) -> List[str]:
    ...
    if model.startswith("deepseek-v4") and "bigmodel" in base:
        hints.append(t("sanity_zai_alias"))
    return hints
...
class CLIConfig:
    ...
    api_key: str = ""
    model: str = "default"
    # SEC-002：默认 readonly。原默认是 write，而 WRITE_TOOLS 里含 terminal_exec /
    # file_delete / code_execute —— 权限门一过就再无逐次确认，等于把最危险的三个工具
    # 默认放开。提权用 /perm write 或 --permission write 显式声明。
                    ...
                    for key in SKILLS:
                        if key.startswith(sk_prefix):
                            yield Completion(
                                ...
                                # 走 t() 而不是 SKILLS[key]["name"]：@skill 执行那条路
                                # 已经查语言包了，这里直读数据会让补全菜单和执行结果
                                # 对同一个技能给出两种叫法（en/ja 下菜单还是中文）。
                                display_meta=t(f"skill_{key}"),
                            )
                    return
                prefix = text[1:]
...
def merge_config(args) -> Dict:
    ...
    try:
        ...
        print(c("yellow", t("config_validate_warn", err=e)))
    return cfg


def save_cli_config(cfg: Dict) -> None:
    CONFIG_PATH.write_text(json.dumps(cfg, ensure_ascii=False, indent=2), encoding="utf-8")
    print(c("green", t("config_saved", path=CONFIG_PATH)))
...
class ModelClient:
    ...
    def stream_generate(self, system: str, messages: List[Dict],
        ...
        if not self.base_url or not self.api_key:
            raise RuntimeError(t("model_not_configured"))
    ...
    def _stream_openai(self, system: str, messages: List[Dict],
        ...
        import requests
        # 两个**独立**的预算，不要混成一个计数器：
        #   tools 降级只该发生一次（端点不支持就是不支持，再试也一样）；
        #   空流重开是网络抖动的补救，可以多来一次。
        # 合并成单个循环变量会导致"降级用掉了一次机会，抖动就没机会了"。
        stream_restarts_left = 2
        tools_downgrades_left = 1
        while True:
            payload = {
                "model": self.model,
            ...
            try:
                # 状态阶段（连接 + 响应头）的 429/5xx/连接抖动由 ace_http 退避重试兜住。
                # 它只覆盖到拿到状态码为止 —— 响应体在下面才消费，中途断流得单独处理。
                ...
                if (self.tools_ok and tools_downgrades_left > 0
                ...
                # 中途断流。**只有还没吐出任何内容时才允许重开**：
                # 内容一旦流给了用户（on_delta 或 print），重发会把前半段再吐一遍，
                # 而模型第二次的措辞不会和第一次一致，用户看到的是自我矛盾的拼接文本。
                if full == "" and not tool_calls and stream_restarts_left > 0:
                    stream_restarts_left -= 1
                    continue
                raise
            if tool_calls:
            ...
            return full
        raise RuntimeError(t("model_api_failed"))
    ...
    def _post_anthropic(self, payload: Dict,
        ...
        # Anthropic 的过载码 529 不在 HTTP 标准里，但实际会返回；
        # ace_http.RETRYABLE_STATUS 已经把它算进可重试集合。
        r = ace_http.request_with_retry(
        ...
        for payload in self._anthropic_payload_variants(system, messages):
            try:
                ...
                try:
                    ...
                    pass
                last_err = t("model_api_resp_body", err=e, body=body)
                ...
                try:
                    ...
                    # 判据是服务端返回的 code / message，不是我们自己的文案 ——
                    # 这里匹配的"不存在"是对方的原话，不会随界面语言变。
                    if code == "1214" or "modelCode" in msg or "不存在" in msg:
                        last_err += t("model_hint_bad_model_name")
                except Exception:
                    pass
                if e.response is not None and e.response.status_code != 400:
                    # 只有 400 值得换请求格式重试 —— 它才可能是"这个服务商不认这种 payload 形状"。
                    # 401/403 是凭据问题，429/5xx 已经在 _post_anthropic 里被 ace_http
                    # 退避重试过并且用尽了预算，在这里再换格式重发只是白打请求。
                    break
            except requests.RequestException as e:
                last_err = str(e)
                break
        raise RuntimeError(t("model_api_failed_formats", err=last_err))
...
class AgentCLI:
    ...
    def _init_execution_layer(self) -> None:
        self.el = ExecutionLayer(
            ...
            config={
                ...
                # 交互式入口必须把审批回调接上：否则判定为 prompt 的命令（git commit、
                # 带管道的命令）会被无条件 403，安全闸门就变成了功能墙。
                "approval_hook": make_cli_approval_hook(),
                # SEC-010：README 的配置示例里一直有 signing_key，但这里以前只往下传
                # 三个键，于是快照 HMAC 从未启用 —— 用户照文档配了也不生效、且没有
                # 任何提示。没有签名，模型可以用项目内相对路径改写
                # .guardian/snapshots/<id>/meta.json，伪造一份自洽的元信息，
                # verify_snapshot 会放行，而 rollback 的第 3 步会先删掉当前全部文件。
                "signing_key": self.cfg.get("signing_key"),
                "signing_key_path": self.cfg.get("signing_key_path"),
                "sign_snapshots": self.cfg.get("sign_snapshots", True),
                "max_snapshots": self.cfg.get("max_snapshots", 20),
            },
        )
        # metadata 的"只给人"通道（实现见 agent_runner）。装在这里而不是 __init__：
        # /clear、/permission 都会重建执行层，装在构造点才不会在重建后失效 ——
        # 失效的表现是"某次 500 之后细节就再也不打了"，没人会想到去查开关。
        DETAIL_TAP.install(self.el)
        # 配了但没生效必须说出来。静默失效比明确报错更危险：用户以为有防护。
        if getattr(self.el, "signing_key_warning", ""):
            print(c("yellow", f"⚠ {self.el.signing_key_warning}"))
    ...
    def _build_system_prompt(self) -> str:
        ...
        parts.append(f"【用户环境】用户主目录: {_home}；用户桌面目录: {_desktop}。"
                     ...
                     f"桌面与下载目录默认可读；其他项目外目录每次读取都会向用户单独确认，"
                     f"密钥类文件（.env、id_rsa、credentials 等）一律拒绝——"
                     f"被拒时不要换工具或换写法重试，直接告知用户。")
        ...
        if self.context_refs:
            ...
            parts.append("【已引用上下文】\n" + "\n".join(
                wrap_untrusted(ref, source="用户 @ 引用的文件/目录内容",
                               origin="at_ref", nonce=self._ctx_nonce) for ref in self.context_refs))
        return "\n\n".join(parts)
    ...
    def _at_file(self, arg: str) -> None:
        ...
        try:
            ...
            return
        if len(content) > 4000:
            # 不走 t()：context_refs 只进系统提示词（见 _build_system_prompt），
            # _at_refs 展示时只取首行标题，这个标记永远到不了用户眼前 ——
            # 它是给模型看的"后面被我截掉了"，按约定给模型的文本保持固定中文。
    ...
    def _at_folder(self, arg: str) -> None:
        ...
        try:
            ...
            return
        if len(items) > 30:
            items = items[:30] + ["…(更多)"]   # 同 @file：模型侧文本，不 i18n
        self.context_refs.append(f"📁 {p}\n" + "\n".join(items))
        self.context_refs = self.context_refs[-3:]
    ...
    def converse(self, user_input: str, echo_input: bool = True) -> None:
        ...
        stall = StallTracker(STALL_ABORT_ROUNDS)
        for _round in range(1, MAX_ROUNDS + 1):
            ...
            # metadata 的"只给人"通道：每轮都取走（取走即清空），否则本轮没细节时
            # 会把上一轮那个文件的路径打给用户 —— 指错位置比不指更糟。
            # 打印点刻意放在状态行**之后**（见下面两处），不然"位置 / 系统原话"
            # 会出现在它要解释的那一行前面。
            round_detail = DETAIL_TAP.take()

            # 连续失败/无进展熔断：判定逻辑在 agent_runner.StallTracker，
            # 与 agent_runner 入口共用同一套语义（改一处两个入口都对）。
            ...
            if result["status"] == "PLAN_PROPOSED":
                ...
                if sys.stdin.isatty():
                    ...
                    # 原实现这里是 answer = "y"，而几行之下的权限申请分支是 answer = "n" ——
                    # 同一段代码里两个确认点的失败方向相反，说明是疏漏而非设计。
                    print(c("dim", t("auto_deny_plan")))
                    answer = "n"
                if answer in ("y", "yes"):
            ...
            if result["status"] in ("FORMAT_ERROR", "GUARD_VIOLATION",
                ...
                for _detail_line in execution_detail_lines(round_detail,
                                                           result["status"]):
                    print(c("dim", _detail_line))
                ...
                print(line)
                # 5xx 也落在这条分支（这里的错误状态元组不含 500）：SUCCESS 的
                # metadata 只有 elapsed 之类，取不到"位置 / 系统原话"，自然不打印。
                for _detail_line in execution_detail_lines(round_detail,
                                                           result["status"]):
                    print(c("dim", _detail_line))
                if result["status"] == "SUCCESS":
    ...
    def run_command(self, cmd: str) -> bool:
        ...
        if name == "/help":
            ...
            if not snaps:
                print(t("snap_none"))
            for s in snaps:
                print(t("snap_line", id=s['id'], iso=s.get('created_iso'),
                        tag=s.get('tag'), n=s.get('file_count')))
        elif name == "/undo":
            self._undo_last()
        elif name == "/rollback":
            if len(parts) < 2:
                print(t("rollback_usage"))
            elif not re.match(r"^\d+_[\w\-]{1,60}$", parts[1]):
                print(c("red", t("rollback_bad_id")))
            else:
                try:
                    answer = input(t("rollback_confirm", id=parts[1])).strip().lower()
                except (EOFError, KeyboardInterrupt):
                    print()
                    print(t("cancelled"))
                    return True
            ...
            print(c("green", t("report_done", path=path)) if path
                  else c("red", t("report_disabled")))
        elif name == "/permission":
            if len(parts) >= 2 and parts[1] in ("readonly", "write", "full"):
                self.el.permission.upgrade(parts[1])
                self.cfg["permission"] = parts[1]
                print(c("green", t("perm_switched", level=parts[1])))
            else:
    ...
    def _resolve_local_path(self, path_str: str, create: bool = False) -> Optional[Path]:
        ...
        if not p.exists():
            print(c("red", t("path_not_found", path=p)))
            return None
        return p
    ...
    def _open_file(self, path_str: str, prefer_editor: bool = False) -> None:
        ...
        if not path_str.strip():
            print(t("open_usage", cmd="/edit" if prefer_editor else "/open"))
            return
        ...
        try:
            if prefer_editor:
                ...
                if code:
                    ...
                    print(c("green", t("open_in_vscode", path=p)))
                    return
            try:
                if os.name == "nt":
                    os.startfile(str(p))
                ...
                if os.name == "nt" and p.suffix.lower() in _TEXT_EXTENSIONS:
                    try:
                        ...
                        print(c("green", t("open_notepad_fallback", path=p)))
                        return
                    except Exception as e2:
                        print(c("red", t("open_failed_notepad", err=e2)))
                        return
                print(c("red", t("open_failed", err=e)))
                return
            print(c("green", t("open_done", path=p)))
        except Exception as e:
            print(c("red", t("open_failed", err=e)))
    ...
    def _handle_model(self, parts: List[str]) -> None:
        if len(parts) == 1:
            print(t("model_current", desc=self.client.describe()))
            prov = _find_provider(self.cfg)
            if prov:
                print(c("dim", t("model_provider_models",
                                 models=" / ".join(prov["models"][:8]))))
            print(c("dim", t("model_switch_hint")))
            return
        if parts[1] == "base-url" and len(parts) >= 3:
            self.cfg["base_url"] = parts[2]
            save_cli_config(self.cfg)
            self._reload_client()
            print(c("green", t("model_base_url_set", url=parts[2],
                               fmt=detect_api_format(parts[2]))))
        elif parts[1] == "api-key" and len(parts) >= 3:
            self.cfg["api_key"] = parts[2]
            save_cli_config(self.cfg)
            self._reload_client()
            print(c("green", t("model_key_updated", masked=mask_secret(parts[2]))))
        else:
            model_name = " ".join(parts[1:]).strip()
            self.cfg["model"] = model_name
            save_cli_config(self.cfg)
            self._reload_client()
            print(c("green", t("model_switched", name=model_name)))

    def _handle_provider(self, parts: List[str]) -> None:
        """查看/切换 AI 提供商：/provider 列清单，/provider <编号|id> [api-key] 一键切换"""
        if len(parts) == 1:
            print(c("bold", t("provider_list_title")))
            for i, p in enumerate(PROVIDERS, 1):
                cur = self.cfg.get("base_url", "").rstrip("/") == p["base_url"].rstrip("/")
                mark = c("green", t("provider_current_mark")) if cur else ""
                print(f"  {c('magenta', str(i)):>3}. {_provider_label(p)}{mark}")
                print("      " + c("dim", t("provider_line_models",
                                            url=p["base_url"],
                                            models=" / ".join(p["models"][:6]))))
            return
        ...
        if arg.isdigit() and 1 <= int(arg) <= len(PROVIDERS):
            ...
            for p in PROVIDERS:
                if p["id"] == arg:
                    ...
                    break
        if target is None:
            print(c("red", t("provider_unknown", name=parts[1])))
            return
        ...
        save_cli_config(self.cfg)
        self._reload_client()
        print(c("green", t("provider_switched", name=_provider_label(target))))
        print(t("provider_endpoint", url=target["base_url"], fmt=target["api_format"]))
        print(t("provider_model_line", model=self.cfg["model"],
                models=" / ".join(target["models"][:6])))
        if not self.cfg.get("api_key"):
            print(c("yellow", t("provider_no_key")))

    def _config_wizard(self) -> None:
        print(c("bold", t("config_wizard_title")))
        ...
        try:
            # ① 选提供商（一键换端点 + 自动选默认模型）
            print(c("bold", t("config_step_provider")))
            for i, p in enumerate(PROVIDERS, 1):
                print(f"  {i}. {_provider_label(p)}  {c('dim', p['base_url'])}")
            choice = _ask(t("config_ask_provider"), "")
            if choice:
                if choice.isdigit() and 1 <= int(choice) <= len(PROVIDERS):
                    ...
                    print(c("yellow", t("config_bad_index")))
            # ② 密钥（隐藏输入，不回显）
            key = _ask(t("config_ask_key"), mask_secret(self.cfg.get("api_key", "")), hidden=True)
            if key:
            ...
            model_hint = ""
            if prov:
                model_hint = t("config_model_hint",
                               models=" / ".join(prov["models"][:8]))
            model = _ask(t("config_ask_model", hint=model_hint), self.cfg.get("model", ""))
            if model:
                self.cfg["model"] = model
        except CommandCancelled:
            print(c("yellow", t("config_cancelled")))
            return
        save_cli_config(self.cfg)
        self._reload_client()
        print(c("green", t("config_saved_current", desc=self.client.describe())))

    def _handle_cli_mistype(self, line: str) -> None:
        """防蠢处理：识别并接管误打进 REPL 的命令行指令"""
        if "--install-ui" in line:
            print(c("yellow", t("install_ui_detected")))
            if _pip_install_with_fallbacks("prompt_toolkit"):
                print(c("green", t("install_ui_ok")))
            else:
                print(c("red", t("install_ui_fail")))
            return
        print(c("yellow", t("mistype_not_for_agent", line=line.strip())))
        print(c("dim", t("mistype_hint")))
    ...
    def _toggle_mock(self) -> None:
        ...
        if self.client.mock:
            ...
            print(c("green", t("mock_back_real", desc=self.client.describe())))
            if not self.cfg.get("base_url") or not self.cfg.get("api_key"):
                print(c("yellow", t("mock_missing_cfg")))
        else:
            self.client = ModelClient(self.cfg, mock=True)
            print(c("green", t("mock_enabled")))
    ...
    def _undo_last(self) -> None:
        ...
        try:
            ...
            if ok:
                # 时间 / 文件数在键里一起给：以前主句走 t()、尾巴用 f-string 拼中文，
                # en/ja 下就是"半句英文 + 半句中文"，比整句没翻更难在肉眼扫读时发现。
                print(c("green", t("undo_done", id=latest["id"],
                                   iso=latest.get("created_iso"),
                                   n=latest.get("file_count"))))
            else:
    ...
    def repl(self, return_to_landing: bool = False) -> None:
        ...
        if sys.stdin.isatty() and sys.stdout.isatty():
            try:
                ...
                print(c("dim", t("completer_enabled")))
            except ImportError:
                print(c("dim", t("completer_install_hint")))
            except Exception as e:
                # 构造失败（终端/版本兼容等）：降级为普通 input()，但明示原因便于排查
                print(c("yellow", t("completer_failed",
                                    err=f"{type(e).__name__}: {e}")))
                session = None
        else:
            print(c("dim", t("completer_non_tty")))
...
def main() -> None:
    # 语言必须在**任何输出之前**定下来。--install-ui / merge_config 的校验提示都在
    # 拿到 AgentCLI（那里才 set_language）之前就可能打印，不先设就会先甩一屏中文
    # 再切语言。语言源是配置文件而不是命令行参数，所以这一步不依赖 parse_args ——
    # 反过来也成立：正因为没有 --lang 参数，才不存在"要先解析才知道语言"的死结。
    try:
        set_language(str(load_cli_config().get("lang", "zh")))
    except ValueError:
        pass   # 配置里写了不支持的语言：保持默认 zh，具体报错留给 CLIConfig 校验

    # description / help 故意不做 i18n：见 locales 提交说明。只在 `ace --help` 出现，
    # 且参数名本身是英文，翻了也只是半英半译；真要做，成本是 15 个键 × 3 语言。
    ...
    parser.add_argument("--context-window", type=int, default=0,
                        help="模型上下文窗口 token 数，用于压缩阈值（默认 32768）")
    parser.add_argument("--no-compact", action="store_true",
                        help="关闭上下文压缩（超长时退回硬截断，会丢早期对话）")
    ...
    if args.install_ui:
        print(t("install_ui_running"))
        if _pip_install_with_fallbacks("prompt_toolkit"):
            print(t("install_ui_ok"))
        else:
            print(t("install_ui_fail"))
        return
