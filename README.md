...
- 🔑 **权限申请**：工具被 403 拦截时模型可申请临时授权（`request_permission`），用户可选「本次」或「本会话」；`terminal_exec` 只允许本次
...
- 回滚安全网自保：`.guardian/` 快照目录对所有工具只读且不可写删（它就在 agent 可写的项目目录里，不挡住的话改一行 `meta.json` 就能让熔断回滚静默失效）；回滚失败会告警而不是静默吞掉
...
- 授权分两档：「本次」用后即焚，「本会话」本会话内不再询问；`terminal_exec` 强制逐次确认，不接受会话级授权。回滚仅回滚本轮快照
